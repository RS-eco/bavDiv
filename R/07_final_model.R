#' ---
#' title: "Final GLMM of species richness in Bavaria"
#' author: "RS-eco"
#' ---
...
library(glmmTMB)
if(!file.exists(paste0(filedir, "/data/lmer_all.rda"))){
  ...
  save(lmer_all, file=paste0(filedir, "/data/lmer_all.rda"), compress="xz")
} else{
  load(paste0(filedir, "/data/lmer_all.rda"))
}
if(!file.exists(paste0(filedir, "/data/lmer_aves.rda"))){
  lmer_aves <- glmmTMB(sum_art ~ jahrIndex + mon + 
                         perc_cov + jahrIndex:perc_cov + 
                         (1 | KARTE_QUAD) + (1 | Haupt_Nr),
                       ziformula=~0, data=dat_aves, family=poisson, 
                       REML = FALSE, weights=sum_n)
  save(lmer_aves, file=paste0(filedir, "/data/lmer_aves.rda"), compress="xz")
} else{
  load(paste0(filedir, "/data/lmer_aves.rda"))
}
if(!file.exists(paste0(filedir, "/data/lmer_lepi.rda"))){
  lmer_lepi <- glmmTMB(sum_art ~ jahrIndex + mon + 
                         perc_cov + jahrIndex:perc_cov + 
                         (1 | KARTE_QUAD) + (1 | Haupt_Nr),
                       ziformula=~0, data=dat_lepi, family=poisson, 
                       REML = FALSE, weights=sum_n)
  save(lmer_lepi, file=paste0(filedir, "/data/lmer_lepi.rda"), compress="xz")
} else{
  load(paste0(filedir, "/data/lmer_lepi.rda"))
}
if(!file.exists(paste0(filedir, "/data/lmer_odo.rda"))){
  lmer_odo <- glmmTMB(sum_art ~ jahrIndex + mon + 
                        perc_cov + jahrIndex:perc_cov + 
                        (1 | KARTE_QUAD) + (1 | Haupt_Nr),
                      ziformula=~0, data=dat_odo, family=poisson, 
                      REML = FALSE, weights=sum_n)
  save(lmer_odo, file=paste0(filedir, "/data/lmer_odo.rda"), compress="xz")
} else{
  load(paste0(filedir, "/data/lmer_odo.rda"))
}
if(!file.exists(paste0(filedir, "/data/lmer_ortho.rda"))){
  lmer_ortho <- glmmTMB(sum_art ~ jahrIndex + mon + 
                          perc_cov + jahrIndex:perc_cov + 
                          (1 | KARTE_QUAD) + (1 | Haupt_Nr),
                        ziformula=~0, data=dat_ortho, family=poisson, 
                        REML = FALSE, weights=sum_n)
  save(lmer_ortho, file=paste0(filedir, "/data/lmer_ortho.rda"), compress="xz")
} else{
  load(paste0(filedir, "/data/lmer_ortho.rda"))
}
...
#' **Figure 1.** Effect sizes of generalized linear mixed effects model of species richness. <!-- Significances to labels??? -->
...
#' **Figure 2.** Modelled change in species richness over time for min, median and max percentage cover protection.
